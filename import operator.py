...
class Particle:
    ...
    def display(self):
        pygame.draw.circle(screen, self.colour, (self.x+offset,offset-self.y), self.size, self.thickness)
...
vx, vy = [float(i)*100 for i in x2.split()]
print(vx,vy)
...
while running:
    ...
    # p = v.normalize_return_length()
    # print(m,d,p)
    # v=round((vx**2+vy**2)**.5,4)
    # acc=-.8*m/t
    background.fill(background_colour)
    text = font.render(f"p0 :{round(p0.x/100,2)} {round(p0.y/100,2)} speed:{round(p0.speed)},p1 :{round(p1.x/100,2)} {round(p1.y/100,2)} speed:{round(p1.speed)}", True, (10, 10, 10))
